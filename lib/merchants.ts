...
export async function getActiveMerchants(): Promise<Merchant[]> {
  ...
  return snap.docs
    ...
    return {
      ...
      boostPurchasedAt: data.boostPurchasedAt ?? undefined,
...
export async function searchMerchants(params: SearchMerchantsParams): Promise<MerchantResult[]> {
  ...
  filtered = filtered.map((m) => {
    ...
    // Boost no longer affects sort order — merchants keep orange border + fire badge
    // but are sorted by proximity like everyone else.
